...
<script>
$(document).on('click', 'table.selected-table tbody tr', function() { $(this).toggleClass('selected'); });

$(document).on('change', '.rebuildTable', function() { rebuildTable(); });
...
<script>
function selectAllRowTable(params) {
    event.preventDefault();
    var selection = $(rendorTableBodyOn+" tr");
    if (params == true) { selection.addClass("selected") }
    else if (params == false) { selection.removeClass("selected") }
}
